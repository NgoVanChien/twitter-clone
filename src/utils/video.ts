...
export const checkVideoHasAudio = async (filePath: string) => {
  ...
  const { stdout } =
    await $`ffprobe -v error -select_streams a:0 -show_entries stream=codec_type -of default=nw=1:nk=1 ${filePath}`
...
export const encodeHLSWithMultipleVideoStreams = async (_inputPath: string) => {
  const slash = (await import('slash')).default
  const inputPath = slash(_inputPath)
  // console.log(slash(inputPath))
  const [bitrate, resolution] = await Promise.all([getBitrate(inputPath), getResolution(inputPath)])
  const _parent_folder = path.join(_inputPath, '..')
  const _outputSegmentPath = path.join(_parent_folder, 'v%v/fileSequence%d.ts')
  const _outputPath = path.join(_parent_folder, 'v%v/prog_index.m3u8')
  const parent_folder = slash(_parent_folder)
  const outputSegmentPath = slash(_outputSegmentPath)
  const outputPath = slash(_outputPath)
